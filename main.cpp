...
int main ()
{
  ...
  **/
  ...
  pid_steer.Init(0.4, 0.001, 0.5, 1.2, -1.2,2); 
  //pid_steer.Init(0.44, 0.003, 0.55, 1.2, -1.2,2); 
  
  pid_throttle.Init(0.15, 0.004, 0.1, 1, -1,2);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          double delta_dist = (new_delta_time * velocity);
          printf("delta dist %f \n",(new_delta_time * velocity));
          prev_timer = timer;
